# Privilege Escalation Detections

**MITRE ATT&CK Tactic:** [Privilege Escalation (TA0004)](https://attack.mitre.org/tactics/TA0004/)
**Kill Chain Phase:** Exploitation

Detections for techniques adversaries use to gain higher-level permissions, including exploitation of vulnerabilities in services and applications, abuse of elevated process tokens, and living-off-the-land privilege escalation chains.

---

## Detections

| Detection | MITRE Technique | Description |
|-----------|----------------|-------------|
| [Cisco SD-WAN CVE-2026-20245 CLI Tenant-Upload Injection](cisco_sdwan_cve_2026_20245_tenant_upload_cli_injection.md) | T1068, T1190, T1136, T1078, T1070 | CVE-2026-20245 (CVSS 7.8): authenticated SD-WAN CLI tenant-upload command injects OS commands as root via vconfd_script_upload_tenant_list.sh; troot UID 0 backdoor account creation; active exploitation disclosed by Mandiant June 24 2026 |
| [Microsoft ADFS CVE-2026-56155 Insufficient Access Control EoP](adfs_cve_2026_56155_access_control_eop.md) | T1068, T1484.002, T1078.002, T1550.001, T1059.001 | CVE-2026-56155 (CWE-1220): authorized attacker abuses ADFS access control granularity flaw to obtain tokens for unauthorized relying parties or modify RPT/claims configuration to elevate privileges; CISA KEV added 2026-07-14 |
| [Microsoft SharePoint Server CVE-2026-56164 Missing Auth EoP](sharepoint_cve_2026_56164_missing_auth_eop.md) | T1190, T1098, T1548, T1136 | CVE-2026-56164 (CWE-306): unauthenticated attacker calls privileged SharePoint REST endpoints without credentials to elevate to site admin; CISA KEV added 2026-07-14 with 3-day deadline |
| [Oracle EBS CVE-2026-46817 Payments Unauthenticated Privilege Escalation](oracle_ebs_cve_2026_46817_payments_privesc.md) | T1190, T1548, T1083, T1552.001 | CVE-2026-46817 (CVSS 9.8): unauthenticated POST to /OA_HTML/ibytransmit invokes internal Oracle Java function to read arbitrary server files; auth bypass on Oracle Payments File Transmission; CISA KEV added 2026-07-15 with July 18 deadline |
| [FalconFlank — CrowdStrike Falcon Sensor LPE via Cleanup Routine Abuse](falconflank_crowdstrike_falcon_sensor_lpe.md) | T1068, T1562.001, T1548 | Public PoC (Chaotic Eclipse, 2026-09-03) for LPE in CrowdStrike Falcon Sensor cleanup service (CsFalconService.exe); no CVE/patch at time of writing; detects shell spawned from CsFalconService.exe and symbolic link creation near Falcon paths |
| [Windows ALPC and Update Stack LPE Zero-Days (CVE-2026-85880, CVE-2026-81963)](windows_alpc_update_stack_lpe_zero_day.md) | T1068, T1134 | Two actively exploited Windows LPE zero-days patched September 2026 (CISA KEV Sep 8): CVE-2026-85880 (ALPC heap buffer overflow, AppContainer escape to SYSTEM) and CVE-2026-81963 (Windows Update Stack link following, low-priv to SYSTEM); detects Update Stack processes spawning shell interpreters |
| [Veeam Agent for Windows gRPC Named Pipe LPE (CVE-2026-32996)](veeam_agent_grpc_named_pipe_lpe_cve_2026_32996.md) | T1068, T1490 | CVE-2026-32996 (CVSS 7.3): low-privileged local attacker sends gRPC control messages over `\\.\pipe\Veeam\VAW\ServiceConnectionPipe` to escalate to SYSTEM via VeeamAgentSvc.exe; active exploitation confirmed by Arctic Wolf (Sep 17+) in ransomware pre-encryption backup destruction chains; CISA KEV added 2026-09-21 |

## Threat Actors

| Actor | Techniques | References |
|-------|-----------|-----------|
| Unattributed (state-sponsored suspected) | T1068, T1190, T1136 | [Mandiant GTIG — CVE-2026-20245 (June 2026)](https://cloud.google.com/blog/topics/threat-intelligence/zero-day-exploitation-cisco-catalyst-sd-wan-manager) |
| COZY BEAR / APT29 | T1550.001, T1484.002 | [MITRE ATT&CK G0016](https://attack.mitre.org/groups/G0016/) — known ADFS targeting for Golden SAML and federated identity attacks |
| SCATTERED SPIDER / Octo Tempest | T1484.002, T1078.002 | [MITRE ATT&CK G1015](https://attack.mitre.org/groups/G1015/) — active targeting of federated identity infrastructure |
| Unknown (CVE-2026-56155 AD FS exploiters, July 2026) | T1068 | [CISA KEV (2026-07-14)](https://www.cisa.gov/known-exploited-vulnerabilities-catalog), [Tenable — Microsoft July 2026 Patch Tuesday](https://www.tenable.com/blog/microsofts-july-2026-patch-tuesday-addresses-569-cves-cve-2026-56155-cve-2026-56164) |
| Multiple (CISA KEV confirmed exploitation) | T1190, T1098 | CVE-2026-56164 SharePoint Missing Auth EoP; attribution not yet disclosed |
| Unattributed (CVE-2026-46817 Oracle EBS exploiters, June–July 2026) | T1190, T1548, T1083 | [The Hacker News — CVE-2026-46817 (June 2026)](https://thehackernews.com/2026/06/oracle-e-business-suite-flaw-cve-2026.html), [CISA KEV (2026-07-15)](https://www.cisa.gov/known-exploited-vulnerabilities-catalog) |
| Chaotic Eclipse (Nightmare-Eclipse) | T1068, T1548 | [The Register — FalconFlank PoC (2026-09-03)](https://www.theregister.com/security/2026/09/03/prolific-microsoft-0-day-hunter-drops-crowdstrike-falcon-exploit-poc/5294318), [Abstract Security Detection Guidance](https://www.abstract.security/blog/chaotic-eclipse-releases-crowdstrike-falcon-zero-day-falconflank-detection-guidance) |
| Unknown (CVE-2026-85880 / CVE-2026-81963 exploiters, September 2026) | T1068 | [CISA KEV (2026-09-08)](https://www.cisa.gov/news-events/alerts/2026/09/08/cisa-adds-four-known-exploited-vulnerabilities-catalog), [Tenable September 2026 Patch Tuesday](https://www.tenable.com/blog/microsofts-september-2026-patch-tuesday-addresses-964-cves-cve-2026-81963-cve-2026-85880) |
| Multiple ransomware operators / Unknown (CVE-2026-32996 exploiters, September 2026) | T1068, T1490 | [CISA KEV (2026-09-21)](https://www.cisa.gov/known-exploited-vulnerabilities-catalog), [Arctic Wolf — Active Exploitation](https://arcticwolf.com/resources/blog/) |
