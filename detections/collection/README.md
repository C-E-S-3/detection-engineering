...
| [TELEPUZ Chrome DevTools Protocol (CDP) Browser Session Hijacking](telepuz_cdp_browser_session_hijacking.md) | T1185, T1555.003 | Chrome/Edge launched with `--remote-debugging-port` by non-testing parent process (e.g., PowerShell, cmd.exe); or unrecognized process connecting to localhost CDP port range (9222–9229); TELEPUZ MaaS web injector uses CDP/WebDriver BiDi to inject JavaScript into active banking sessions without a browser extension |
...
| TELEPUZ MaaS Operator | Financially Motivated Cybercrime (MaaS) | Modular MaaS delivered via ClickFix-VIDAR chain; WebSocket WSS C2 with Polygon blockchain fallback (EtherHiding); CDP/WebDriver BiDi web injector intercepts banking sessions without a browser extension; targets financial sector North America/Western Europe; active H1–Q3 2026 | [Elastic Security Labs — TELEPUZ (2026-07-16)](https://www.elastic.co/security-labs/telepuz-maas-malware-clickfix) |
